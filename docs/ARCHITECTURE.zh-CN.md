...
- `reducer`：负责可插拔算法、共享 runtime service、candidate planning/model 类型、接受、回退、cache 使用和共享
  finishing stage。
...
- `runtime/`：负责端到端 engine、算法可用 context、candidate trial runner、oracle 前 objective、cache、state、reporting
  和 validation。
- `model/`：负责 reducer 数据类型，包括 candidate、edit plan、group、strategy 和 trial record。
- `planning/`：负责语言无关 candidate planning、ddmin chunk、ordering、edit 提取、retarget 和按结构区域重分组。
- `shared_stages/`：负责所有算法都会运行的最终阶段，目前是经过验证的 blank-line cleanup。
- `algorithms/structured/`：负责确定性分阶段 scheduler，并拆成 fixed-point round、单个 stage invocation、final sweep
  和 stage-order helper。
- `algorithms/weighted_random/`：负责自适应随机 scheduler，并按 point collection、weighted sampling、PRNG、cleanup
  sweep 和 rename sweep 拆分。

`reducer::candidate`、`reducer::engine`、`reducer::phase` 等 compatibility module 会从这些子目录重导出主要公开
类型/函数，让既有 API 路径保持稳定。
...
- 可选的 size stop target 只基于当前 accepted source 检查，不基于 rejected candidate 检查。
...
`trials/current` 会被 baseline confirmation、candidate trial 和 final confirmation 复用。current trial
directory 以及固定的 side A/B 工作目录只会在 session 开始时打印一次。完整 candidate source 只保留当前 A/B
输入和上一轮 accepted source。每次尝试还会写入 `trials/current/current.diff`，并在 `history/<trial>.diff`
中保存相对上一轮 accepted source 的历史 diff。
...
因为 side 目录固定且已经在启动时打印，每次 attempt 的开始日志不会重复这些路径。
...
`structured` 是默认的确定性 reducer。它的目录中，`fixed_point` 负责普通 stage 轮次，`stage` 负责一次会重新生成
候选的 stage loop，`final_sweep` 负责单候选 cleanup，`stage_order` 负责 final sweep 的 stage 列表。下面会描述它的
stage。
...
- `AggressiveFunctionElimination` 基于 function summary 和 call site 工作。它会先只 neutralize best-effort
  callee name 与目标 function name 精确匹配的外部 call site。无法识别 callee 的调用不会被挂到每个函数上，
  而是留给后续 statement 和 expression 阶段。function declaration 会作为后置的 declaration-only single
  candidate 单独尝试，并且只有在精确外部 call site 已经消失、函数名也不再作为完整 identifier 出现在其他位置时
  才会生成。
...
function declaration 删除是 broad chunking 的例外。它使用 single-candidate group，避免 reducer 反复尝试
容易编译失败的 call-site/declaration 混合 chunk。
...
命令 process group 发送 SIGTERM，等待 1 秒，再向仍在这些 group 中的进程发送 SIGKILL，然后让 reducer
完成 graceful output。reducer 观察到 signal 后不会再启动新的子进程；它会写出最后一个 accepted source 和可选
JSON report，跳过 final confirmation，再用约定的 signal 退出码退出。命令 timeout 也使用同样的 SIGTERM、
等待 1 秒、SIGKILL 流程清理对应 process group。
