...
`config::ReducerLimits` 保存 `max_rounds`、`max_trials` 和可选的 `SizeStopConfig`。CLI 会把用户传入的
round/trial `0` 转成 `usize::MAX`，表示不设置显式上限。

`config::SizeStopConfig` 支持两个互相独立的目标大小停止条件：
...
process group 执行同样的清理流程。收到 shutdown signal 后，reducer 会停止启动新的子进程，写出最后一个
accepted source 和可选 JSON report，然后以 `128 + signal` 退出。
...
和 literal cleanup。function call-site neutralization 是保守的：只有 best-effort callee/function name
精确匹配的 call site 才会被放进目标 function group；function declaration 删除会在精确外部 call site 消失后，
作为 single-candidate 工作单独生成。
...
删除 function declaration 的 group 使用 `SinglesOnly` 调度。这会把 declaration removal 和 call-site
neutralization 分开，避免大量通常会编译失败的混合 chunk。
...
5. 运行选定的 reducer 算法；
6. 运行共享的最终 `BlankLineCleanup` 阶段；
7. 写入最小化输出；
8. 未收到 shutdown signal 时运行最终确认；
9. 写入可选 JSON report。

如果收到 Ctrl+C 或 SIGTERM，engine 会停止调度新候选，跳过 final confirmation，将最后一个已 accepted
的 snapshot 写入 output path，写入可选 JSON report，并返回带有 signal 标记的 summary。

engine 与语言无关。它只处理 `LanguageAdapter`、`ProgramSnapshot`、`CandidateGroup`、`EditPlan`、
`ComplexityScore` 和 oracle decision。runtime 实现位于 `reducer::runtime`：`engine` 负责 session 生命周期，
`context` 负责算法可用 API，`trial_runner` 负责一次 candidate attempt，`objective` 负责 oracle 前简化目标检查。
candidate 数据类型位于 `reducer::model`；candidate planning、retarget、ordering 和按结构区域重分组位于
`reducer::planning`。算法实现位于 `reducer::algorithms`，并通过 `ReductionContext` 访问共享的候选生成、
...
- 配置的 `stop_size_bytes` 和 `stop_size_percent`；
- baseline/final diff 状态；
- per-phase report；
- trial-limit、size-limit 和 shutdown-signal 状态；
- 可用时的 kept workspace directory。

`interrupted_by_signal` 在正常 run 中为 `null`。当它是整数时，output source 是 shutdown 之前最后一个已经验证
accepted 的 snapshot；因为 signal 之后不会再启动新子进程，report 可能不包含 final confirmation 产生的效果。
