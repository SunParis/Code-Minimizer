...
- `--stop-size <size>`：当当前 accepted source 小于等于指定大小时停止继续化简。纯数字表示 bytes；
  也支持 `B`、`KB`、`MB` 和 `GB` 后缀。
- `--stop-size-percent <n>`：当当前 accepted source 小于等于原始源码大小的 `n`% 时停止继续化简。
  可以写成 `50` 或 `50%`。
...
`AggressiveFunctionElimination` 会拆成更保守的子步骤。call-site neutralization group 只包含 best-effort
callee name 与目标 function name 精确匹配的外部 call site。无法识别 callee 的调用不会被分配给所有函数，
而是留给后续 statement/expression 阶段处理。function declaration 会在后面作为 declaration-only 的
single-candidate group 单独尝试，并且只有在没有精确外部 call site、函数名也不再作为完整 identifier 出现在其他
位置时才生成。
...
current trial directory 和 side A/B 工作目录；之后每个 build/run 的开始日志不再重复这些路径。拒绝日志会使用
可读 phase 名和稳定 enum 名，并包含 group、attempt、candidate 数量和 oracle 原因。

## Java 和 JavaScript 支持

JavaScript 候选包括注释清理、输出调用删除、保守的 function/顶层声明删除、block 和 statement 化简、
控制流替换、表达式替换、literal 缩小和 cleanup edit。

Java 候选包括注释清理、import 删除、保守的 function 和 type declaration 删除、成员和局部声明删除、
输出调用删除、block 和方法体替换、控制流替换、表达式替换、literal 缩小、空嵌套 block 删除和 cleanup edit。
Java adapter 默认保护唯一 public 顶层类。
...
- 配置的 stop-size byte 和 percentage target；
- 是否因为 trial limit、size target 或 shutdown signal 停止；
...
再发送 SIGKILL。之后它会写出最后一个已 accepted 的源码和可选 JSON report，再用约定的 signal
退出码退出。这个中断路径会跳过 final confirmation，避免 shutdown 期间再启动新的子进程。单条命令 timeout
时也使用同样的子进程清理流程。
